...
1. **검수 및 발행**: 관리자가 수집된 `DRAFT` 공연을 검토하고 `PUBLISHED` 상태로 전환
2. **대상 추출**: 해당 공연의 아티스트를 팔로우한 사용자 목록 추출
3. **알림 전송**: FCM을 통해 기기별 푸시 알림 발송
4. **이동**: 사용자가 알림 클릭 시 공연 상세 페이지로 바로 이동

### 시나리오 4: 공연 데이터 생명주기 관리 (상태 동기화)

1. **상태 추적**: 백그라운드 스케줄러가 현재 노출 중인(`PUBLISHED`) 모든 공연을 모니터링
2. **KOPIS 동기화**: 매일 새벽 KOPIS API를 호출하여 최신 공연 상태(`status`) 확인
3. **상태 업데이트**: '공연중', '공연완료' 등 변경 사항을 DB에 즉시 반영
4. **종료**: '공연완료' 상태가 된 공연은 이후 자동 동기화 대상에서 제외 및 관리
